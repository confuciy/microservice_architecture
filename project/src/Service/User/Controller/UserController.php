...
class UserController
{
    ...
    public function edit()
    {
        try {
            ...
            echo '<div style="padding: 10px;">';
                ...
                | <a href="/user/profile">Профиль пользователя</a>
                | <b>Редактирование профиля пользователя</b>
                | <a href="/user/order">Заказы</a>
                | <a href="/user/billing">Биллинг-аккаунт</a>
                | <a href="/user/notification">Уведомления</a>
    ...
    public function profile()
    {
        try {
            ...
            echo '<div style="padding: 10px;">';
                ...
                | <a href="/user/edit">Редактирование профиля пользователя</a>
                | <a href="/user/order">Заказы</a>
                | <a href="/user/billing">Биллинг-аккаунт</a>
                | <a href="/user/notification">Уведомления</a>
                | <a href="/user/exit">Выход</a>
        ...
        echo '<div style="padding: 10px;">';
            ...
            function createOrder(idempotency, reorder) {
                ...
                Object.keys(warehouse_item_list).forEach(function(key) {  
                    ...
                    if(warehouse_item_list[key].name && warehouse_item_list[key].count > 0) { 
                ...
                if (count_total > 0) {
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        type: "POST",
                        data: ({ "warehouse_list": JSON.stringify(order_item_list), "reorder": reorder }),
                        async: false,
                        xhrFields: {
                            withCredentials: true // Ключевая опция для отправки куки
                        },
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.order_check == false) {
                                ...
                                //alert("Подобный заказ был уже существует!");
                                if (confirm("Подобный заказ уже существует!\n\nВы желаете создать повторный заказ?")) {
                                
                                    createOrder(data.idempotency, 1);
                                } 
                            
                            // Создание заказа
                            } else {
                            
                                createOrder(data.idempotency, 0);
                            }
                        },
                        error: function (xhr) {
    ...
    public function billing()
    {
        ...
        echo '<div style="width: 100%; background: lightgrey;">
                <div style="padding: 10px;">
                    <h1>arch.homework / Биллинг-аккаунт</h1>
                </div>
            </div>';
