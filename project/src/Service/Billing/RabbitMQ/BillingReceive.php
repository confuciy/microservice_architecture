        ...
        try {
            ...
            if (isset($msg_data['action']) and $msg_data['action'] == 'create') {
                ...
                if (isset($msg_data['data']) and sizeof($msg_data['data']) > 0) {
                    ...
                    echo " [✓] Биллинговый аккаунт c ID = ".$billing['billing_id']."  успешно создан\n";

                    $helper->setNotification($msg_data['data']['user_id'], 'rabbitmq-billing-receive', '[✓] Биллинговый аккаунт c ID = '.$billing['billing_id'].' успешно создан');
                }
            }
